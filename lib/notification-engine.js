...
// ── Admin Alert Rate Limiter (token bucket) ─────────────────
const ADMIN_ALERT_TOKENS = parseInt(process.env.TELEGRAM_ADMIN_ALERT_TOKENS || '10', 10);
const ADMIN_ALERT_WINDOW = parseInt(process.env.TELEGRAM_ADMIN_ALERT_WINDOW_MS || '60000', 10);
let adminAlertTokens = ADMIN_ALERT_TOKENS;
let adminAlertLastRefill = Date.now();

function checkAdminAlertRate() {
  const now = Date.now();
  const elapsed = now - adminAlertLastRefill;
  const refill = Math.floor(elapsed / (ADMIN_ALERT_WINDOW / ADMIN_ALERT_TOKENS));
  if (refill > 0) {
    adminAlertTokens = Math.min(ADMIN_ALERT_TOKENS, adminAlertTokens + refill);
    adminAlertLastRefill = now;
  }
  if (adminAlertTokens > 0) {
    adminAlertTokens -= 1;
    return true;
  }
  return false;
 ...
 * Rate-limited separately from user-facing messages (10 alerts/min default).
 */
export async function sendTelegramAdminAlert(notification) {
  ...
  if (!checkAdminAlertRate()) {
    logger.warn(`[NOTIFICATION] Admin alert rate limited, skipping: ${notification.title}`);
    return;
  }
