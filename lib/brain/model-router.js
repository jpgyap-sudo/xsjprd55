...
// Wired to: existing AI providers (Kimi, Claude, OpenAI, Ollama)
// ============================================================
...
export async function explainDecision({ context, strategy, risk }) {
  ...
  if (provider === 'ollama') {
    return explainViaOllama({ context, strategy, risk });
  }
...
/**
 * Explain via Ollama (local LLM).
 * Uses the Ollama REST API at OLLAMA_BASE_URL (default http://localhost:11434).
 * Model is configured via OLLAMA_MODEL (default: phi3:mini for low-RAM VPS).
 * Falls back to local explanation if Ollama is unreachable.
 */
async function explainViaOllama({ context, strategy, risk }) {
  const baseUrl = (process.env.OLLAMA_BASE_URL || 'http://localhost:11434').replace(/\/+$/, '');
  const model = process.env.OLLAMA_BRAIN_MODEL || process.env.OLLAMA_MODEL || 'phi3:mini';

  const prompt = loadPrompt('signal-reviewer.md');
  const systemMsg = prompt || 'You are a trading signal analyst. Provide concise analysis.';
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`[model-router] Ollama HTTP ${response.status}`);
      return explainLocal({ context, strategy, risk });
    }

    const data = await response.json();
    const explanation = data?.message?.content || 'No explanation from Ollama';
    return {
      provider: 'ollama',
      model,
      explanation,
    ...
    console.error('[model-router] Ollama error:', err.message);
    return explainLocal({ context, strategy, risk });
  }
}
