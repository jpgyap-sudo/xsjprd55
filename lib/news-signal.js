...
import { fetchNewsFromStore } from './news-store.js';
...
const WATCH_ASSETS = ASSET_MAP; // All assets including LTC, BCH, UNI, AAVE, SUI, SEI, ARB, OP
...
export async function scanNewsSignals(options = {}) {
  ...
  try {
    // 1. Fetch news from persistent store (ingested by /api/news-ingest cron)
    const newsItems = await fetchNewsFromStore(maxAgeMinutes, 200);
