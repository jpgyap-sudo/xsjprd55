...
export async function healStrategies() {
  ...
  for (const [strategy, groupSignals] of Object.entries(byStrategy)) {
    ...
    const suggestion = {
      ...
      suggestion: generateHealingSuggestion(strategy, winRate, avgPnl, groupSignals),
  ...
  logger.info(`[strategy-healer] Processed ${healed} underperforming strategies (${healed - (healed - (healed - (await getMockHealedCount())))})`);
  return healed;
}
...
async function healMockStrategies() {
  ...
  for (const [strategy, groupSignals] of Object.entries(byStrategy)) {
    ...
    const suggestion = {
      ...
      suggestion: `Mock strategy "${strategy}" has ${(winRate * 100).toFixed(0)}% win rate (avg PnL: $${avgPnl.toFixed(2)}). ${needsQuarantine ? 'Disable and replace.' : 'Adjust entry filters or reduce leverage.'}`,
      healed_at: new Date().toISOString(),
    };
 ...
 */
function generateHealingSuggestion(strategy, winRate, avgPnl, signals) {
  const sideBreakdown = {};
  for (const s of signals) {
  ...
  // Check if one side is significantly better
  let sideAdvice = '';
