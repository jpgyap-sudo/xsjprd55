// ============================================================
// Aggressive Mock Trading Engine v4 — ML-Powered Perpetual Trader
...
// v4: Enhanced exit logic via exit-engine.js (partial TP, time stop, breakeven).
// ============================================================
...
import { enhancedCloseLogic } from './exit-engine.js';
...
export async function monitorAndCloseAggressive() {
  ...
  for (const trade of openTrades) {
    try {
      ...
      // Enhanced exit logic (partial TP, time stop, breakeven)
      let enhancedExit = null;
      if (config.ENABLE_EXIT_ENGINE) {
        try {
          enhancedExit = enhancedCloseLogic(trade, price);
        } catch (e) {
          logger.warn(`[AGGRESSIVE-ENGINE] Enhanced exit failed for ${trade.symbol}: ${e.message}`);
        }
      }
      ...
      else if (enhancedExit?.shouldExit) exitReason = enhancedExit.reason;
...
export async function closeAggressiveTrade(tradeId, exitPrice, exitReason, meta = {}) {
  ...
  // Update the account that owns this trade. Falling back to any/default
  // aggressive account can make dashboard balances look frozen or wrong.
  let account = null;
  if (trade.account_id) {
    const { data: tradeAccount, error: accountErr } = await supabase
      ...
      .maybeSingle();
    if (accountErr) logger.warn(`[AGGRESSIVE-ENGINE] Trade account fetch failed: ${accountErr.message}`);
    account = tradeAccount;
  }
  if (!account) account = await getOrCreateAggressiveAccount();
  ...
  if (accErr) {
    logger.warn(`[AGGRESSIVE-ENGINE] Account update failed (retrying minimal): ${accErr.message}`);
    const { error: minimalErr } = await supabase.from('mock_accounts').update({
      current_balance: newBalance,
    }).eq('id', account.id);
    if (minimalErr) {
      logger.error(`[AGGRESSIVE-ENGINE] Minimal account update also failed: ${minimalErr.message}`);
    }
  }
  ...
  try {
    ...
    await recordMockTradeOutcome(
      ...
      { source: 'aggressive_engine' }
    );
  } catch (e) {
    logger.debug('[AGGRESSIVE-ENGINE] TLL outcome recording skipped:', e.message);
  }
