...
// Trades ALL Binance perpetuals. Self-adjusts leverage via ML.
...
export async function getOrCreateAggressiveAccount() {
  const { data: existing } = await supabase.from('mock_accounts').select('*').eq('name', 'Aggressive AI Trader').limit(1).maybeSingle();
...
export async function openAggressiveTrade(signal, opts = {}) {
  ...
  const { data, error } = await supabase.from('mock_trades').insert({
    ...
    metadata: { tvSummary, leverageReason: levReason },
  }).select('*').single();
...
async function learnFromLoss(trade, pnl) {
  ...
  // Insert into loss_patterns for ML analysis
...
export async function getAllPerpetualSymbols() {
  try {
    ...
    logger.warn(`[AGGRESSIVE-ENGINE] Failed to load markets: ${e.message}`);
