...
export async function getOrCreateExecutionAccount() {
  ...
  if (insertError) {
    ...
    logger.warn('[EXEC] Insert account error:', insertError.message);
  }

  // 5. Ultimate fallback: ephemeral account (trades will fail FK, but worker stays alive)
  logger.error('[EXEC] Could not persist account — mock_trades will fail FK constraint. Run seed SQL: supabase/seed-mock-accounts.sql');
  return {
