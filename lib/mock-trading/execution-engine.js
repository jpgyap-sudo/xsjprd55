...
export async function getOrCreateExecutionAccount() {
  ...
  // Fall back to any account with >0 balance
  if (!existing) {
  ...
  const { data, error } = await supabase.from('mock_accounts').insert({
    ...
    metadata: { version: 'v3_execution', strategy: 'risk_reward_optimizer' },
  }).select('*').single();
...
export async function openExecution(signal, evaluation) {
  ...
  const { data, error } = await supabase.from('mock_trades').insert({
    ...
    score_breakdown: { evaluation, profile, kellyFraction, riskPct },
    metadata: { execution_version: 'v3', source_signal: signal },
  }).select('*').single();
