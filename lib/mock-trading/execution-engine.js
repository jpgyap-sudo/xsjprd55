...
export async function evaluateSignalForExecution(signal) {
  ...
  if (rrRatio < MIN_RR_RATIO) {
    return { execute: false, reason: `R/R ${rrRatio.toFixed(2)} < min ${MIN_RR_RATIO}` };
  }
  ...
  try {
    ...
    if (mlResult.confidence && mlResult.signal !== 'NO_MODEL') {
      ...
      if (mlProb < 0.45) return { execute: false, reason: `ML confidence ${mlProb.toFixed(2)} too low` };
  ...
  try {
    ...
    if (!aligned && signal.confidence < 0.75) {
