...
export async function getOrCreateExecutionAccount() {
  const knownCols = 'id, name, starting_balance, current_balance, created_at';
  ...
  // Supabase may return [] or [{...}] — handle both
  const account = Array.isArray(data) ? data[0] : data;
  if (account) return account;

  // Final fallback: fetch what we just inserted
  const { data: fetched } = await supabase
    .from('mock_accounts')
    .select(knownCols)
    .eq('name', 'Execution Optimizer v3')
  ...
  throw new Error('Failed to create or fetch execution account after insert');
}
