...
export async function getOrCreateExecutionAccount() {
  // Use * select to avoid column-mismatch issues with RLS or schema drift
  const knownCols = '*';

  // Try v3 account first
  ...
  // Insert without .select() first (safer when RLS blocks read-after-insert)
  ...
  if (insertError) {
    ...
    console.error('[EXEC] Insert account error:', insertError.message);
    // Don't throw — try to fetch anyway in case insert worked but returned nothing
  }

  // Fetch what we just inserted (or existing)
  const { data: fetched } = await supabase
    .from('mock_accounts')
    .select(knownCols)
    .eq('name', 'Execution Optimizer v3')
  ...
  // Ultimate fallback: create an in-memory account so worker doesn't crash
  console.warn('[EXEC] Could not persist account — using ephemeral in-memory account');
  return {
    id: 'ephemeral-exec-v3',
    name: 'Execution Optimizer v3',
    starting_balance: STARTING_BALANCE,
    current_balance: STARTING_BALANCE,
...
export async function evaluateSignalForExecution(signal) {
  ...
  try {
    ...
    if (mlResult.confidence && mlResult.signal !== 'NO_MODEL') {
      ...
      const mlThreshold = (signal.mode === 'paper' || !signal.mode) ? 0.35 : 0.45;
      if (mlProb < mlThreshold) return { execute: false, reason: `ML confidence ${mlProb.toFixed(2)} too low (threshold ${mlThreshold})` };
  ...
  try {
    ...
    const tvThreshold = (signal.mode === 'paper' || !signal.mode) ? 0.85 : 0.75;
    if (!aligned && signal.confidence < tvThreshold) {
...
export async function closeExecution(tradeId, exitPrice, exitReason, meta = {}) {
  ...
  await supabase.from('mock_trades').update({
    ...
    metadata: { ...trade.metadata, closeMeta: meta },
  }).eq('id', tradeId);
