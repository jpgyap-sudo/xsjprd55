...
// ── Account ─────────────────────────────────────────────────
const MIN_BALANCE_RESET_THRESHOLD = 1; // Reset if balance drops below $1

async function ensureAccountBalance(account) {
  const balance = Number(account.current_balance ?? account.starting_balance ?? STARTING_BALANCE);
  if (balance >= MIN_BALANCE_RESET_THRESHOLD) return account;

  logger.warn(`[EXEC] Account balance $${balance.toFixed(2)} is critically low — resetting to $${STARTING_BALANCE.toLocaleString()}`);
  ...
  if (error) {
    logger.error(`[EXEC] Failed to reset account balance: ${error.message}`);
    return account;
  }

  logger.info(`[EXEC] Account balance reset to $${STARTING_BALANCE.toLocaleString()}`);
  return { ...account, current_balance: STARTING_BALANCE, peak_balance: STARTING_BALANCE, realized_pnl: 0 };
}

export async function getOrCreateExecutionAccount() {
  const safeCols = 'id, name, starting_balance, current_balance, peak_balance, realized_pnl, created_at';
  try {
    ...
    if (existing) {
      ...
      // Auto-reset if balance is critically low
      return await ensureAccountBalance(existing);
    }

    // 3. Minimal insert with only guaranteed columns
    const minimalData = {
    ...
    // 4. Duplicate → fetch existing
    if (insertErr) {
      logger.warn('[EXEC] Insert account error:', insertErr.message, insertErr.code);
  ...
  logger.error('[EXEC] Run this SQL in Supabase: INSERT INTO mock_accounts (name, starting_balance, current_balance) VALUES (\'Execution Optimizer v3\', 1000000, 1000000);');
  return null;
}
...
export async function evaluateSignalForExecution(signal) {
  ...
  try {
    ...
    if (!account) {
      logger.debug('[EXEC] RL check skipped: no account available');
    } else {
      ...
      const rlResult = await getRlDecision(
        ...
        { balance, open_positions: openCount, today_pnl: todayPnl, max_drawdown_pct: await getMaxDrawdownPct(account) }
      );
      // In paper mode, never reject on RL skip — log and continue
      const isPaper = config.TRADING_MODE === 'paper' || !config.TRADING_MODE;
      if (rlResult.decision?.action === 'skip') {
        if (isPaper) {
          logger.debug(`[EXEC] RL recommends skip for ${signal.symbol} in paper mode — ignoring to allow learning`);
        } else {
  ...
  const account = await getOrCreateExecutionAccount();
  if (!account) {
    return { execute: false, reason: 'No trading account available for daily loss check' };
  }
  const todayLoss = Math.abs(await getTodayPnl(account.id));
  const balance = Number(account.current_balance ?? account.starting_balance ?? STARTING_BALANCE);
  const dailyLossPct = (todayLoss / balance) * 100;
  // In paper mode, warn but do not block — allow learning from all signals
  const isPaper = config.TRADING_MODE === 'paper' || !config.TRADING_MODE;
  if (dailyLossPct > DAILY_LOSS_CUTOFF_PCT) {
    if (isPaper) {
      logger.debug(`[EXEC] Daily loss ${dailyLossPct.toFixed(1)}% exceeds cutoff in paper mode — continuing for learning`);
    } else {
...
export async function openExecution(signal, evaluation) {
  const account = await getOrCreateExecutionAccount();
  if (!account) {
    return { error: 'No trading account available. Check Supabase connection and mock_accounts table.' };
  }
  const balance = Number(account.current_balance ?? account.starting_balance ?? STARTING_BALANCE);
  ...
  await logTradeHistory({
    ...
    metadata: { evaluation, profile, kellyFraction, riskPct }
  });
...
export async function closeExecution(tradeId, exitPrice, exitReason, meta = {}) {
  ...
  let account = null;
  if (trade.account_id) {
    const { data: tradeAccount, error: accountErr } = await supabase
      .from('mock_accounts')
      .select('id, name, starting_balance, current_balance, peak_balance, realized_pnl, created_at')
      .eq('id', trade.account_id)
      .maybeSingle();
    if (accountErr) logger.warn(`[EXEC] Trade account fetch failed: ${accountErr.message}`);
    account = tradeAccount;
  }
  if (!account) account = await getOrCreateExecutionAccount();

  const newBalance = Number(account.current_balance ?? account.starting_balance ?? STARTING_BALANCE) + pnl.pnlUsd;
  const peak = Math.max(Number(account.peak_balance || newBalance), newBalance);
  const newRealizedPnl = Number(account.realized_pnl || 0) + pnl.pnlUsd;

  const { error: accErr } = await supabase.from('mock_accounts').update({
  ...
  if (accErr) {
    logger.warn(`[EXEC] Account update failed (retrying minimal): ${accErr.message}`);
    const { error: minimalErr } = await supabase.from('mock_accounts').update({
      current_balance: newBalance,
    }).eq('id', account.id);
    if (minimalErr) {
      logger.error(`[EXEC] Minimal account update also failed: ${minimalErr.message}`);
    }
  }
  ...
  // ── Feed outcome into TLL (non-blocking) ─────────────────
  try {
    const { recordMockTradeOutcome } = await import('../learning-layer/mock-trading-bridge.js');
    await recordMockTradeOutcome(
      { ...trade, pnl_pct: pnl.pnlPct, pnl_usd: pnl.pnlUsd, exit_price: exitPrice, exit_reason: exitReason, closed_at: new Date().toISOString(), status: 'closed' },
      { source: 'execution_engine' }
    );
  } catch (e) {
    logger.debug('[EXEC] TLL outcome recording skipped:', e.message);
  }
...
async function getMaxDrawdownPct(account) {
  ...
  return startingBalance > 0 ? (maxDD / startingBalance) * 100 : 0;
}
