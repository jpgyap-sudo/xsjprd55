...
/**
 * Synchronous compatibility helper used by the aggressive mock trader.
 * Returns an exit suggestion without writing to Supabase.
 * @param {object} trade
 * @param {number} currentPrice
 * @returns {{shouldExit:boolean, reason?:string, exitPrice?:number, rMultiple?:number}}
 */
export function enhancedCloseLogic(trade, currentPrice) {
  const entry = parseFloat(trade.entry_price);
  const stopLoss = parseFloat(trade.stop_loss);
  if (!entry || !currentPrice || !stopLoss) return { shouldExit: false };

  const side = String(trade.side || '').toUpperCase();
  const isLong = side === 'LONG';
  const riskPerUnit = Math.abs(entry - stopLoss);
  if (riskPerUnit === 0) return { shouldExit: false };

  const movePerUnit = isLong ? currentPrice - entry : entry - currentPrice;
  const currentR = movePerUnit / riskPerUnit;
  const timeStop = checkTimeStop(trade, currentPrice, currentR);
  if (timeStop.shouldClose) {
    return {
      shouldExit: true,
      reason: 'time_stop',
      exitPrice: timeStop.exitPrice,
      rMultiple: currentR,
    };
  }

  return { shouldExit: false, rMultiple: currentR };
}
