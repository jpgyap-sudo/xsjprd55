...
export async function getOrCreateMockAccount() {
  ...
  // 3. Insert without .select() first (safer when RLS blocks read-after-insert)
  ...
  // 5. Ultimate fallback: ephemeral in-memory account
  logger.warn('[MOCK-ACCOUNT] Could not persist account — using ephemeral in-memory account');
  return {
