...
export async function openMockTrade(signal, probability) {
  ...
  const stopLoss = signal.side === 'long' ? entry * (1 - stopLossPct / 100) : entry * (1 + stopLossPct / 100);
  const takeProfit = signal.side === 'long' ? entry * (1 + takeProfitPct / 100) : entry * (1 - takeProfitPct / 100);
