...
// EMA Cross, RSI Bounce, Volume Filter + Neural Social Intel
// All parameters read from environment with sensible defaults.
// ============================================================

import { v4 as uuidv4 } from 'uuid';
import { getEventsForSignalWindow } from './social-intel-store.js';
...
const SOCIAL_INTEL_WEIGHT = Number(process.env.SOCIAL_INTEL_WEIGHT || 0.12);

// ── Social Intel Integration ────────────────────────────────
export async function getSocialIntelForSymbol(symbol, minutes = 60) {
  try {
    const events = await getEventsForSignalWindow({ symbol, minutes });
    if (!events.length) return { multiplier: 0, events: [], dominantBias: 'neutral' };
    ...
    for (const ev of events) {
      const weight = (ev.event_score || 0.5) * (ev.confidence || 0.5);
      totalScore += (ev.sentiment_score || 0) * weight;
      totalWeight += weight;

      if (ev.suggested_bias === 'bullish') bullishCount++;
      if (ev.suggested_bias === 'bearish') bearishCount++;
    }

    const avgSentiment = totalWeight > 0 ? totalScore / totalWeight : 0;
    const multiplier = avgSentiment * SOCIAL_INTEL_WEIGHT; // e.g. +0.05 to -0.05
    const dominantBias = bullishCount > bearishCount ? 'bullish' : bearishCount > bullishCount ? 'bearish' : 'neutral';

    return { multiplier, events, dominantBias, avgSentiment: Number(avgSentiment.toFixed(3)) };
  } catch (error) {
    logger.warn(`[signal-engine] getSocialIntelForSymbol failed: ${error.message}`);
    return { multiplier: 0, events: [], dominantBias: 'neutral' };
  }
}
...
export function strategy_EMACross(pair, tf, ohlcv) {
  ...
  const atrVal = last(atrValues) || (Math.max(...closes.slice(-14)) - Math.min(...closes.slice(-14)));
...
// ── Run with Social Intel Boost ─────────────────────────────
export async function runAllStrategiesWithIntel(pair, tf, ohlcv) {
  const signals = runAllStrategies(pair, tf, ohlcv);
  if (!signals.length) return [];

  const symbol = pair.replace('/', '');
  const { multiplier, events, dominantBias, avgSentiment } = await getSocialIntelForSymbol(symbol, 60);

  for (const sig of signals) {
    // Boost or reduce confidence based on social sentiment alignment
    const alignment =
      (sig.side === 'LONG' && dominantBias === 'bullish') ? 1 :
      (sig.side === 'SHORT' && dominantBias === 'bearish') ? 1 :
      (sig.side === 'LONG' && dominantBias === 'bearish') ? -1 :
      (sig.side === 'SHORT' && dominantBias === 'bullish') ? -1 :
      0;

    const adjustment = alignment * Math.abs(multiplier);
    const originalConfidence = sig.confidence;
    sig.confidence = Number(Math.max(0.35, Math.min(0.99, sig.confidence + adjustment)).toFixed(3));

    // Tag signal with social intel metadata
    sig.metadata = {
      ...sig.metadata,
      social_intel: {
    ...
    logger.info(`[signal-engine] ${sig.symbol} ${sig.side} confidence=${sig.confidence} (was ${originalConfidence}, social_adj=${adjustment.toFixed(4)}, bias=${dominantBias})`);
  }
