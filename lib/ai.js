...
// Tertiary: Ollama (local LLM) via REST API
...
// ── Ollama (local LLM) ────────────────────────────────────
async function callOllama({ messages, maxTokens = 4096 }) {
  const baseUrl = (process.env.OLLAMA_BASE_URL || 'http://localhost:11434').replace(/\/+$/, '');
  const model = process.env.OLLAMA_CHAT_MODEL || process.env.OLLAMA_MODEL || 'phi3:mini';

  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 8000); // 8s timeout — fail fast if Ollama not running
    ...
    return { ok: false, error: err.message || 'Ollama connection failed' };
  }
}

// ── Main askAI — auto-selects provider with fallback ──────
export async function askAI({ question, chatHistory = [], maxTokens = 4096, timeoutMs = 25000 }) {
  ...
  // Total timeout — fail fast so server 30s timeout doesn't kill us silently
  const timeoutPromise = new Promise((_, reject) =>
    setTimeout(() => reject(new Error(`askAI timed out after ${timeoutMs}ms`)), timeoutMs)
  );
...
User question: ${question}`;
      ...
      if (provider === 'kimi') {
        ...
        // Tertiary fallback to Ollama if both Kimi and Anthropic fail
        console.warn('[AI] Anthropic also failed, falling back to Ollama:', anthropicResult.error);
        const ollamaMessages = [
          { role: 'system', content: SYSTEM_PROMPT },
          ...historyMessages,
          { role: 'user', content: contextText }
        ];
        const ollamaResult = await callOllama({ messages: ollamaMessages, maxTokens });

        if (ollamaResult.ok) {
          return buildResponse(ollamaResult, marketContext, newsContext);
        }

        return { ok: false, error: `Kimi: ${kimiResult.error}; Anthropic: ${anthropicResult.error}; Ollama: ${ollamaResult.error}` };
      }
      ...
      if (provider === 'ollama') {
        ...
        console.warn('[AI] Ollama failed, falling back to Kimi:', ollamaResult.error);
        const kimiMessages = [
          { role: 'system', content: SYSTEM_PROMPT },
          ...historyMessages,
          { role: 'user', content: contextText }
        ...
        return { ok: false, error: `Ollama: ${ollamaResult.error}; Kimi: ${kimiResult.error}` };
      }
      ...
      // Fallback to Ollama if Anthropic fails
      console.warn('[AI] Anthropic failed, falling back to Ollama:', anthropicResult.error);
      const ollamaMessages = [
        { role: 'system', content: SYSTEM_PROMPT },
        ...historyMessages,
        { role: 'user', content: contextText }
      ...
      return { ok: false, error: `Anthropic: ${anthropicResult.error}; Ollama: ${ollamaResult.error}` };
    })(),
    timeoutPromise,
  ]);
}
...
export async function generateAIResponse(systemPrompt, userPrompt, opts = {}) {
  ...
  // Try providers in order: preferred → fallback1 → fallback2 (Ollama last)
  const providers = preferred === 'kimi'
    ? [tryKimi, tryClaude, tryOllama]
    : preferred === 'ollama'
      ? [tryOllama, tryKimi, tryClaude]
      : [tryClaude, tryKimi, tryOllama];
