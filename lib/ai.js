...
async function fetchLiquidationIntel() {
  try {
    // Prefer explicit APP_URL, fallback to VERCEL_URL, then localhost
    const base = config.APP_URL || (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : `http://localhost:${config.PORT}`);
