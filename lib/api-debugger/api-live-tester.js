...
export async function testKimiApi() {
  ...
  const baseUrl = config.KIMI_BASE_URL || 'https://api.moonshot.ai/v1';
...
export async function testBinanceApi() {
  ...
  const requestSafe = { endpoint, method: 'GET', note: 'public endpoint, no key needed' };

  const result = await safeFetch(endpoint, { method: 'GET' }, 15000);
...
export async function testInternalEndpoints() {
  ...
  for (const ep of endpoints) {
    ...
    // Health endpoints intentionally return 503 when some downstream services are degraded
    // but still provide valid JSON. Treat as degraded, not down.
    const isHealthEndpoint = ep.path === '/api/health' || ep.path === '/api/diagnostics';
    const isDegraded = isHealthEndpoint && result.status === 503;
    let parsed = null;
    try { parsed = JSON.parse(result.text); } catch { /* ignore */ }
    const hasValidBody = parsed && parsed.ok === true;

    const classified = (result.ok || isDegraded)
      ? null
    ...
    results.push({
      ...
      endpoint: url,
      method: ep.method,
      status: result.ok ? 'healthy' : (isDegraded && hasValidBody ? 'degraded' : (result.status === 0 ? 'timeout' : 'down')),
      http_code: result.status || 0,
      response_time_ms: result.durationMs,
      error_category: classified?.category || null,
      error_message: isDegraded && hasValidBody
        ? 'Some downstream services degraded (e.g. exchange timeout)'
        : (result.error?.message || (!result.ok ? result.text?.slice(0, 300) : null) || null),
      request_safe: { path: ep.path, method: ep.method },
      response_safe: (result.ok || isDegraded) ? { preview: result.text?.slice(0, 200) } : { raw_preview: result.text?.slice(0, 200) },
      severity: isDegraded ? 'medium' : (result.ok ? 'info' : (classified?.severity || 'medium'))
    });
  }
  return results;
}
...
export async function runAllApiLiveTests() {
  const [kimi, claude, binance, bybit, internal] = await Promise.allSettled([
    testKimiApi(),
    testClaudeApi(),
    testBinanceApi(),
    testBybitApi(),
  ...
  if (binance.status === 'fulfilled') out.push(binance.value);
  else out.push({ provider: 'binance', status: 'error', error_message: binance.reason?.message || 'Promise rejected', severity: 'high' });

  if (bybit.status === 'fulfilled') out.push(bybit.value);
  else out.push({ provider: 'bybit', status: 'error', error_message: bybit.reason?.message || 'Promise rejected', severity: 'high' });
