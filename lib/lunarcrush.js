...
// Note: LunarCrush API version may change. Try api4 first, fall back to api3.
const BASE_URL = (process.env.LUNARCRUSH_API_BASE || 'https://lunarcrush.com/api4').replace(/\/+$/, '');
...
async function lunarFetch(path, query = {}) {
  ...
  const urls = [
    `${BASE_URL}${path}${qs ? '?' + qs : ''}`,
    // Fallback: try api3 if api4 fails
    BASE_URL.includes('api4') ? `${BASE_URL.replace('api4', 'api3')}${path}${qs ? '?' + qs : ''}` : null,
  ].filter(Boolean);
  ...
  for (const url of urls) {
    try {
      ...
      lastError = new Error(`LunarCrush ${res.status}: ${txt}`);
      // If it's a 401 (bad key) or 403, don't retry
      if (res.status === 401 || res.status === 403) break;
    } catch (e) {
      lastError = e;
    }
  }
  throw lastError || new Error('LunarCrush fetch failed');
}
