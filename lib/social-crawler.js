...
// Dynamic playwright import — gracefully handle missing package
let playwrightChromium = null;
let playwrightLoadAttempted = false;

async function getPlaywrightChromium() {
  if (playwrightLoadAttempted) return playwrightChromium;
  playwrightLoadAttempted = true;
  try {
    const mod = await import('playwright');
    playwrightChromium = mod.chromium;
    logger.info('[social-crawler] Playwright loaded successfully');
  } catch (error) {
    logger.warn('[social-crawler] Playwright unavailable, using fetch fallback:', error.message);
...
async function crawlPlaywrightSource(source, maxItems) {
  ...
  if (!chromium) {
    logger.warn(`[social-crawler] Playwright not available, skipping playwright source: ${source.id}`);
    return [];
  }
