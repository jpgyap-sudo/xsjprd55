...
export async function getBugFixPipelineStats() {
  const { data: criticalNew, error: e1 } = await supabase
    .from('bugs_to_fix')
    .select('id', { count: 'exact', head: true })
    .eq('severity', 'critical')
    .in('status', ['new', 'investigating']);

  const { data: highNew, error: e2 } = await supabase
    .from('bugs_to_fix')
    .select('id', { count: 'exact', head: true })
    .eq('severity', 'high')
    .in('status', ['new', 'investigating']);
  ...
  const { data: fixed, error: e4 } = await supabase
    .from('bugs_to_fix')
    .select('id', { count: 'exact', head: true })
    .eq('status', 'fixed');
  ...
  return {
    criticalPending: criticalNew?.length ?? 0,
    highPending: highNew?.length ?? 0,
