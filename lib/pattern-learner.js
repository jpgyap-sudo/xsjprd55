...
export async function extractPattern(signal, opts = {}) {
  if (!signal || typeof signal !== 'object') {
    throw new Error('extractPattern: signal is null or not an object');
  }
  if (!signal.id || !signal.symbol || !signal.side) {
    throw new Error(`extractPattern: missing required fields on signal (id=${signal.id}, symbol=${signal.symbol}, side=${signal.side})`);
  }
