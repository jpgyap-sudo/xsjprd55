...
// and run the Trading Learning Layer (TLL) for adaptive learning.
// ============================================================
...
import { runIntegratedLearning, runIntegratedTLL, logAgentEvent } from './brain-integration.js';
...
export async function runLearningLoop() {
  ...
  // 6. Run Trading Learning Layer (TLL) — bridge ingestion + pattern discovery + regime detection + skill generation
  try {
    const tllResult = await runIntegratedTLL(results);
    results.tll = {
      // Bridge ingestion results
      perpetualIngested: tllResult.tll?.perpetualIngested || 0,
      backtestIngested: tllResult.tll?.backtestIngested || 0,
      proposalsIngested: tllResult.tll?.proposalsIngested || 0,
      signalMemoryIngested: tllResult.tll?.signalMemoryIngested || 0,
      signalPatternsIngested: tllResult.tll?.signalPatternsIngested || 0,
      feedbackSynced: tllResult.tll?.feedbackSynced || 0,
      perfSynced: tllResult.tll?.perfSynced || 0,
      // Core TLL results
      outcomes: tllResult.tll?.outcomesRecorded || 0,
      patterns: tllResult.tll?.patternsDiscovered || 0,
      regime: tllResult.tll?.regime?.regime || null,
      weights: tllResult.tll?.weightsTuned || 0,
      skills: tllResult.tll?.skillsGenerated || 0,
      healed: tllResult.tll?.strategiesHealed || 0,
    };
    results.brainCycle = {
      reports: tllResult.brain_cycle?.reports?.length || 0,
      suggestions: tllResult.brain_cycle?.suggestions?.length || 0,
      ok: tllResult.brain_cycle?.ok !== false
    };
    await logAgentEvent('learning_loop', 'tll_cycle_complete', {
      existing_outcomes: results.outcomesResolved,
      bridges: {
        perpetual: results.tll.perpetualIngested,
        backtest: results.tll.backtestIngested,
        proposals: results.tll.proposalsIngested,
        signal_memory: results.tll.signalMemoryIngested,
        signal_patterns: results.tll.signalPatternsIngested,
      },
      tll_patterns: results.tll.patterns,
      tll_regime: results.tll.regime,
      tll_skills: results.tll.skills,
      brain_reports: results.brainCycle.reports
    });
  } catch (e) {
    results.errors.push(`tll_brain_cycle: ${e.message}`);
  }
