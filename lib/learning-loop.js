...
import { runIntegratedLearning, logAgentEvent } from './brain-integration.js';
...
export async function runLearningLoop() {
  ...
  // 6. Run Central Brain learning cycle (non-blocking)
  try {
    const brainResult = await runIntegratedLearning(results);
    results.brainCycle = {
      reports: brainResult.brain_cycle?.reports?.length || 0,
      suggestions: brainResult.brain_cycle?.suggestions?.length || 0,
      ok: brainResult.brain_cycle?.ok !== false
    };
    await logAgentEvent('learning_loop', 'brain_cycle_complete', {
      existing_outcomes: results.outcomesResolved,
      brain_reports: results.brainCycle.reports
    });
  } catch (e) {
    results.errors.push(`brain_cycle: ${e.message}`);
  }
