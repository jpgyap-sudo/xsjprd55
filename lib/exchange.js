...
export async function fetchOHLCV(exchangeId, symbol, timeframe, limit = 100) {
  try {
    ...
    // If API key is invalid, retry with spot/public settings (no auth needed)
    if (msg.includes('Invalid Api-Key') || msg.includes('invalid api') || msg.includes('Authentication') || msg.includes('-2015') || msg.includes('-2008')) {
      logger.warn(`[EXCHANGE] API key rejected for ${exchangeId}, switching to public spot endpoint...`);
      try {
        ...
        logger.warn(`[EXCHANGE] Public spot ${exchangeId} also failed: ${pubErr.message}`);
      }
    }
