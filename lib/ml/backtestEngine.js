 ...
 * Uses dummy random-walk candles when no real OHLCV is available.
 * @param {string} symbol
 * @param {number} [candleCount=20]
 * @returns {{results: BacktestResult[], symbol: string}}
 */
export function runBacktestOnProposals(symbol = 'BTCUSDT', candleCount = 20) {
  ...
  // Generate dummy candles if no real data
  const candles = generateDummyCandles(symbol, candleCount);
  ...
  for (const { id, proposal } of proposals) {
    try {
      ...
      if (result) {
        storeBacktestResult(result, symbol);
