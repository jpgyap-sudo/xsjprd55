...
function buildRealFeatures({ symbol, current, slice, btcTrend, volumeChange, avgRange }) {
  const input = {
    symbol,
    price: current.close,
    // Features not derivable from candles alone → neutral (0) instead of Math.random()
    fundingRate: 0,
 ...
 */
export function backtestDynamicStrategy(proposalId, candles, symbol = 'BTCUSDT', opts = {}) {
...
export async function runBacktestOnProposals(symbol = 'BTCUSDT', candleCount = 100) {
  ...
  for (const { id, proposal } of proposals) {
    try {
      const result = backtestDynamicStrategy(id, candles, symbol, { isSynthetic });
      if (result) {
