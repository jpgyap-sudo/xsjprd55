...
import { runDynamicStrategy, loadCandidateProposals, markProposalTested } from './dynamicStrategies.js';
...
export function backtestStrategy(strategyName, candles, symbol = 'BTCUSDT') {
  ...
  const input = {
    ...
    volumeChangePct: (Math.random() - 0.5) * 100,
    volatilityPct: (avgRange / current.close) * 100,
    whaleFlowScore: (Math.random() - 0.5) * 0.6,
    btcTrendScore: (candles[i].close - candles[i - 5].close) / candles[i - 5].close,
