...
// Research Agent → Historical Backtest → Quarantine → Approved → Mock Trader
// v2: Added quarantine_testing status between historically_backtested and approved_for_mock.
// ============================================================
...
/**
 * @typedef {'researched'|'historically_backtested'|'quarantine_testing'|'approved_for_mock'|'mock_testing'|'promoted_candidate'|'rejected'} StrategyStatus
 */
 ...
 * Routes to quarantine_testing instead of direct approval for new strategies.
 * @param {string} strategyName
...
export function recordHistoricalBacktest(strategyName, backtest) {
  ...
  // Route to quarantine_testing instead of direct approved_for_mock
  // This gives new strategies a safety buffer before full mock trading
  const status = approved ? 'quarantine_testing' : 'rejected';
  ...
  db.prepare(`
    ...
    WHERE strategy_name = ?
  `).run(
    status, score, approved ? 1 : 0,
    approved ? null : reasons.join('; '),
    approved ? 30 : 0, // Need 30 mock trades in quarantine
    strategyName
  );
...
/**
 * Transition a strategy from quarantine to mock_testing.
 * Called after quarantine trades meet the threshold.
 * @param {string} strategyName
 */
export function promoteFromQuarantine(strategyName) {
  db.prepare(`
    ...
    SET status = 'approved_for_mock',
        quarantine_passed = 1,
        updated_at = datetime('now')
    WHERE strategy_name = ? AND status = 'quarantine_testing'
  `).run(strategyName);
  logger.info(`[LIFECYCLE] ${strategyName} → approved_for_mock (passed quarantine)`);
}
