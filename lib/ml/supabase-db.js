...
export async function saveStrategyProposal(proposal) {
  const row = {
    ...
    promoted: false,
    rejected: false,
    rules_hash: proposal.rulesHash || null,
    source_name: proposal.sourceName || null,
    source_credibility: proposal.sourceCredibility ?? 0.5,
    is_synthetic: proposal.isSynthetic || false,
  };
  ...
  sqliteDb.prepare(`
    INSERT INTO strategy_proposals (created_at, name, description, rules_json, confidence, tested, promoted, rejected, rules_hash, source_name, source_credibility, is_synthetic)
    VALUES (datetime('now'), ?, ?, ?, ?, 0, 0, 0, ?, ?, ?, ?)
  `).run(row.name, row.description, row.rules_json, row.confidence, row.rules_hash, row.source_name, row.source_credibility, row.is_synthetic ? 1 : 0);
}
...
export async function saveBacktestResult(result) {
  const row = {
    ...
    is_synthetic: result.isSynthetic || 0,
    has_random_features: result.hasRandomFeatures || 0,
  ...
  sqliteDb.prepare(`
    INSERT INTO backtest_results (run_at, strategy_name, symbol, total_return_pct, total_trades, win_rate, sharpe_ratio, max_drawdown_pct, profit_factor, expectancy, is_synthetic, has_random_features, trade_log_json)
    VALUES (datetime('now'), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(row.strategy_name, row.symbol, row.total_return_pct, row.total_trades, row.win_rate, row.sharpe_ratio, row.max_drawdown_pct, row.profit_factor, row.expectancy, row.is_synthetic, row.has_random_features, row.trade_log_json);
}
...
export async function upsertStrategyLifecycle(lifecycle) {
  const row = {
    ...
    promotion_gate_score: lifecycle.promotionGateScore || 0,
    promotion_gate_failures: lifecycle.promotionGateFailures || null,
    source_credibility: lifecycle.sourceCredibility || 0.5,
    source_name: lifecycle.sourceName || null,
    rules_hash: lifecycle.rulesHash || null,
    updated_at: new Date().toISOString(),
  };
  ...
  sqliteDb.prepare(`
    INSERT INTO strategy_lifecycle (proposal_id, strategy_name, status, historical_backtest_score, mock_trading_score, approved_for_mock, rejected_reason, promotion_gate_score, promotion_gate_failures, source_credibility, source_name, rules_hash, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
    ON CONFLICT(strategy_name) DO UPDATE SET
      status = ?, historical_backtest_score = ?, mock_trading_score = ?, approved_for_mock = ?, rejected_reason = ?, promotion_gate_score = ?, promotion_gate_failures = ?, source_credibility = ?, source_name = ?, rules_hash = ?, updated_at = datetime('now')
  `).run(
    ...
    row.promotion_gate_score, row.promotion_gate_failures, row.source_credibility, row.source_name, row.rules_hash,
...
export async function getResearchAgentCounts() {
  ...
  if (USE_SUPABASE) {
    ...
    for (const t of tables) {
      try {
        const { data, error } = await supaQuery(t, 'select', {
          columns: 'id',
          limit: 10000,
        });
        if (error) {
