...
let db;
try {
  // Clean up stale WAL files from crashed processes before opening
  ...
  try {
    if (fs.existsSync(walPath)) {
      ...
      if (walSize > 0) {
        // Check if the main DB is locked — if so, WAL is stale from a crash
        try {
          const testDb = new Database(dbPath, { readonly: true });
          testDb.close();
          // DB opens fine in readonly, WAL is likely stale — remove it
          fs.unlinkSync(walPath);
          console.log(`[ML-DB] Removed stale WAL file: ${walPath}`);
        } catch {
          // DB is locked, WAL is active — leave it alone
        }
      }
    }
    if (fs.existsSync(shmPath)) {
      fs.unlinkSync(shmPath);
      console.log(`[ML-DB] Removed stale SHM file: ${shmPath}`);
    }
  } catch (cleanupErr) {
    console.warn(`[ML-DB] WAL cleanup warning: ${cleanupErr.message}`);
  }

  db = new Database(dbPath);
  db.pragma('journal_mode = WAL');
  // Set a busy timeout so concurrent readers don't crash
...
export function initMlDb() {
  ...
  db.exec(`
    ...
    -- New tables for Promotion Gate v2
    -- Migration: add source_credibility to existing strategy_proposals table
    -- Uses ALTER TABLE IF NOT EXISTS pattern (safe to run multiple times)
    PRAGMA table_info(strategy_proposals);
    CREATE TABLE IF NOT EXISTS strategy_proposals_new (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      created_at TEXT NOT NULL,
      name TEXT NOT NULL,
      description TEXT NOT NULL,
  ...
  addColumnIfMissing('strategy_proposals', 'source_credibility', 'REAL DEFAULT 0.5');
  addColumnIfMissing('strategy_proposals', 'is_synthetic', 'INTEGER NOT NULL DEFAULT 0');
  addColumnIfMissing('strategy_proposals', 'rules_hash', 'TEXT');
  addColumnIfMissing('strategy_proposals', 'source_name', 'TEXT');

  // Migration: add missing columns to backtest_results (table may have been created before schema was updated)
  addColumnIfMissing('backtest_results', 'expectancy', 'REAL NOT NULL DEFAULT 0');
  addColumnIfMissing('backtest_results', 'is_synthetic', 'INTEGER NOT NULL DEFAULT 0');
  addColumnIfMissing('backtest_results', 'has_random_features', 'INTEGER NOT NULL DEFAULT 0');

  // Run versioned migrations from lib/ml/migrations/
  const migrationResult = runMigrations(db, { silent: false });
  if (migrationResult.errors.length > 0) {
    console.error(`[ML-DB] Migration errors: ${migrationResult.errors.join('; ')}`);
  }
}
