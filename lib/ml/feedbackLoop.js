...
export async function promoteStrategy(strategyName, metrics) {
  ...
  try {
    await upsertStrategyLifecycle({
      strategyName,
      status: 'promoted',
      historicalBacktestScore: metrics.winRate || 0,
      mockTradingScore: 0,
      approvedForMock: true,
    });
  } catch (e) {
    db.prepare(`
      INSERT INTO strategy_lifecycle (strategy_name, status, historical_backtest_score, mock_trading_score, approved_for_mock, created_at, updated_at)
      VALUES (?, 'promoted', ?, 0, 1, datetime('now'), datetime('now'))
      ON CONFLICT(strategy_name) DO UPDATE SET
        status = 'promoted', historical_backtest_score = MAX(historical_backtest_score, ?), updated_at = datetime('now')
    `).run(strategyName, metrics.winRate || 0, metrics.winRate || 0);
  }
