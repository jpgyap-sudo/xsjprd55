...
import { saveProposal, markSourceUsed } from './saveProposal.js';
...
export async function extractAndSaveFromResearch() {
  // Try Supabase first for unused research sources
  let rows = [];
  try {
    const { getUnusedResearchSources } = await import('./supabase-db.js');
    const supaRows = await getUnusedResearchSources(50);
    if (supaRows && supaRows.length > 0) {
      ...
      logger.info(`[STRATEGY-EXTRACTOR] Loaded ${rows.length} unused sources from Supabase`);
    }
  } catch (e) {
    logger.warn(`[STRATEGY-EXTRACTOR] Supabase fetch failed, falling back to SQLite: ${e.message}`);
  }
  ...
  if (!rows.length) {
    ...
    `).all();
    if (rows.length) {
      logger.info(`[STRATEGY-EXTRACTOR] Loaded ${rows.length} unused sources from SQLite`);
    }
  }

  if (!rows.length) {
    logger.info('[STRATEGY-EXTRACTOR] No unused research sources found');
  ...
  for (const row of rows) {
    ...
    for (const s of strategies) {
      ...
      if (result.ok) {
        ...
        logger.warn(`[STRATEGY-EXTRACTOR] Save failed: ${result.error}`);
      }
    }

    // Mark source as used
    await markSourceUsed(row.id);
  }

  logger.info(`[STRATEGY-EXTRACTOR] Extracted ${extracted} strategies, saved ${saved} proposals`);
  return { extracted, saved, processedSources: rows };
}
