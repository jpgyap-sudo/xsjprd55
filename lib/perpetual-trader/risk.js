...
export function selectLeverage({ confidence, volatilityPct = 2, maxLeverage = 10, defaultLeverage = 3 }) {
  ...
  const volPenalty = volatilityPct >= 3 ? 2 : volatilityPct > 2 ? 1 : 0;
  
  if (confPct < 55) return 0; // Skip
  if (confPct < 65) return Math.max(1, Math.min(2, maxLeverage) - volPenalty);
  if (confPct < 75) return Math.max(1, Math.min(defaultLeverage, maxLeverage) - volPenalty);
  if (confPct < 85) return Math.max(1, Math.min(defaultLeverage + 2, maxLeverage) - volPenalty);
  return Math.max(1, Math.min(defaultLeverage + 4, maxLeverage) - volPenalty);
}
...
export function calculateStops({ entryPrice, side, atr, volatilityPct = 2, riskRewardMin = 1.5 }) {
  const atrDistance = Number.isFinite(atr) && atr > 0 ? atr * 1.5 : null;
  const volatilityDistance = entryPrice * (volatilityPct / 100) * 0.75;
  const stopDistance = atrDistance || volatilityDistance;
...
export function calculateTradeR({ side, entryPrice, currentPrice, stopLoss }) {
...
export function getEnhancedExitDecision({
  side,
  entryPrice,
  currentPrice,
  stopLoss,
  ...
  const baseExit = checkExit({ side, entryPrice, currentPrice, stopLoss, takeProfit });
  if (baseExit.shouldExit) return { ...baseExit, action: 'close' };

  const r = calculateTradeR({ side, entryPrice, currentPrice, stopLoss });
  if (!breakevenMoved && r >= 1) {
    return {
      shouldExit: false,
      action: 'move_breakeven',
      r,
      nextStopLoss: side === 'LONG' ? entryPrice * 1.001 : entryPrice * 0.999,
    };
  }

  if (!partialExitPct && r >= 1) {
    return { shouldExit: false, action: 'partial_take_profit', r, partialExitPct: 0.3 };
  }

  if (r >= 1.5) {
    const proposedTrailing = side === 'LONG'
      ? currentPrice * 0.985
      : currentPrice * 1.015;
    const improvedTrailing = side === 'LONG'
      ? Math.max(trailingStop || 0, proposedTrailing)
      : Math.min(trailingStop || Number.POSITIVE_INFINITY, proposedTrailing);
    return { shouldExit: false, action: 'trail_stop', r, nextTrailingStop: improvedTrailing };
  }

  if (openedAt) {
    const ageMinutes = (Date.now() - new Date(openedAt).getTime()) / 60000;
    if (ageMinutes >= maxHoldMinutes && r < 0.1) {
      return { shouldExit: true, action: 'close', reason: 'expired', exitPrice: currentPrice, r };
    }
  }

  return { shouldExit: false, action: 'hold', r };
}
