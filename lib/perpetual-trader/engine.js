...
export async function getOrCreatePerpetualAccount() {
  ...
  const { data, error } = await supabase
    .from('perpetual_mock_accounts')
    .insert({ name: 'Perpetual Signal Trader', starting_balance: 100000, current_balance: 100000, available_balance: 100000, peak_balance: 100000, equity: 100000 })
    .select()
    .single();
...
export async function closePerpetualTrade(trade, exitPrice, reason, detail = '') {
  ...
  // Update account
  const account = await getOrCreatePerpetualAccount();
  const newBalance = account.current_balance + pnl.pnlUsd;
  const newRealized = account.realized_pnl + pnl.pnlUsd;
  const newDailyPnl = account.daily_pnl_today + pnl.pnlUsd;
  const newAvailable = account.available_balance + trade.margin_used + pnl.pnlUsd;
  const newMarginUsed = Math.max(0, account.margin_used - trade.margin_used);
...
async function updateUnrealizedPnl(account) {
  ...
  const newEquity = account.current_balance + totalUnrealized;
  await supabase.from('perpetual_mock_accounts').update({
