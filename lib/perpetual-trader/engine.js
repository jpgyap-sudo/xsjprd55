...
import { buildPerpetualTradeContext } from './context.js';
import { recordPerpetualTradeOutcome } from './learning.js';
...
export async function openPerpetualTrade(signal) {
  ...
  const tradeContext = await buildPerpetualTradeContext(signal);
  if (!tradeContext.window.allowed) {
    await logTraderEvent(account.id, null, 'warn', 'signal_skip', `Trading window blocked: ${tradeContext.window.reason}`, { signal_id: signal.id, window: tradeContext.window });
    return { ok: false, reason: tradeContext.window.reason };
  }
  if (!tradeContext.regimeCheck.allowed) {
    await logTraderEvent(account.id, null, 'warn', 'signal_skip', `Regime blocked: ${tradeContext.regimeCheck.reason}`, { signal_id: signal.id, regime: tradeContext.regime });
    return { ok: false, reason: tradeContext.regimeCheck.reason };
  }
  if (!tradeContext.throttle.allowed) {
    await logTraderEvent(account.id, null, 'warn', 'signal_skip', `Scorecard throttle blocked: ${tradeContext.throttle.reason}`, { signal_id: signal.id, regime: tradeContext.regime });
    return { ok: false, reason: tradeContext.throttle.reason };
  }
  if ((signal.confidence || 0) < tradeContext.threshold.threshold) {
    const reason = `Confidence ${Math.round((signal.confidence || 0) * 100)}% below dynamic threshold ${Math.round(tradeContext.threshold.threshold * 100)}%`;
    await logTraderEvent(account.id, null, 'warn', 'signal_skip', reason, { signal_id: signal.id, threshold: tradeContext.threshold });
  ...
  const leverage = selectLeverage({
    ...
    volatilityPct: tradeContext.atrPct,
  ...
  const stops = calculateStops({
    entryPrice,
    side: signal.side,
    atr: tradeContext.atr,
    volatilityPct: tradeContext.atrPct || 2,
    riskRewardMin: 1.5,
  });
  ...
  const sizing = calculatePositionSize({
    ...
    leverage,
    maxPositionUsd: account.equity * account.max_exposure_pct * (tradeContext.regimeCheck.sizeAdjustment || 1) * (tradeContext.throttle.isTestTrade ? 0.5 : 1),
  });
  ...
  const { data: trade, error: tradeErr } = await supabase
    ...
    .insert({
      ...
      initial_risk_usd: sizing.riskUsd,
      market_regime_at_entry: tradeContext.regime,
      funding_rate_at_entry: tradeContext.fundingRate,
      ...
      entry_features: {
        ...
        regime_details: tradeContext.regimeDetails,
        funding_rate: tradeContext.fundingRate,
        atr_pct: tradeContext.atrPct,
        dynamic_threshold: tradeContext.threshold.threshold,
      },
      status: 'open',
    })
    .select()
...
export async function monitorPerpetualTrades() {
  ...
  for (const trade of openTrades) {
    ...
    const exit = getEnhancedExitDecision({
      side: trade.side,
      entryPrice: trade.entry_price,
      currentPrice: price,
      stopLoss: trade.stop_loss,
      takeProfit: trade.take_profit,
      openedAt: trade.entry_at || trade.created_at,
      breakevenMoved: Boolean(trade.breakeven_moved),
      trailingStop: trade.trailing_stop,
      partialExitPct: trade.partial_exit_pct || 0,
    });

    if (exit.action === 'move_breakeven') {
      await supabase.from('perpetual_mock_trades').update({
        stop_loss: exit.nextStopLoss,
    ...
    if (exit.action === 'partial_take_profit') {
      await supabase.from('perpetual_mock_trades').update({
        partial_exit_pct: exit.partialExitPct,
    ...
    if (exit.action === 'trail_stop') {
      await supabase.from('perpetual_mock_trades').update({
        trailing_stop: exit.nextTrailingStop,
        stop_loss: exit.nextTrailingStop,
        updated_at: new Date().toISOString(),
      }).eq('id', trade.id);
      continue;
    }
...
export async function closePerpetualTrade(trade, exitPrice, reason, detail = '') {
  ...
  const rMultiple = initialRiskUsd > 0 ? pnl.pnlUsd / initialRiskUsd : 0;
  ...
  await recordPerpetualTradeOutcome({
    ...closedTrade,
    exit_price: exitPrice,
    exit_at: new Date().toISOString(),
  });
