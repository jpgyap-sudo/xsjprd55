...
function normalizeSymbol(raw, exchange) {
  ...
  if (exchange === 'cryptocom') {
    // BTCUSD-PERP, BTC_USD, BTCUSD → BTC
...
async function fetchOkxData() {
  ...
  if (oi?.data) {
    for (const o of oi.data) {
      ...
      oiMap.set(sym, {
        ...
        openInterestUsd: parseFloat(o.oiCcy) || 0
      });
    }
  }
  ...
  for (const [sym, ticker] of tickerMap) {
    ...
    result[sym] = {
      ...
      openInterestUsd: oiData.openInterestUsd || 0,
...
function computeSignals(agg) {
  const coins = Object.values(agg).filter(c => c.price > 0);
  const alerts = [];
  ...
  const bestShort = scored[0];
  scored.sort((a, b) => b.longScore - a.longScore);
  const bestLong = scored[0];
