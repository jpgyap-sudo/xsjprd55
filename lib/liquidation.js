...
function computeSignals(agg) {
  ...
  const scored = coins.map(c => {
    ...
    // OI fuel (only when cross-exchange OI is available; OKX OI removed due to pagination)
    const oiScore = c.openInterestUsd > 0 ? Math.min(Math.log10(c.openInterestUsd + 1) * 4, 15) : 5;
    shortScore += oiScore;
    longScore += oiScore;
    ...
    riskScore += c.openInterestUsd > 0 ? Math.min(Math.log10(c.openInterestUsd + 1) * 2.5, 15) : 5;
  ...
  const bestShort = scored.find(c => c.shortScore >= 8) || scored[0] || null;
  scored.sort((a, b) => b.longScore - a.longScore);
  const bestLong = scored.find(c => c.longScore >= 8) || scored[0] || null;
