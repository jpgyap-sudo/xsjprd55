...
function computeSignals(agg) {
  ...
  const scored = coins.map(c => {
    ...
    // Funding extremes (main signal)
    if (fund > 0.0003) shortScore += Math.min(fund * 3000, 30);
    if (fund < -0.0003) longScore += Math.min(Math.abs(fund) * 3000, 30);
    if (fund > 0.001) shortScore += 20;
    if (fund < -0.001) longScore += 20;
    if (fund > 0.003) shortScore += 15; // extreme
    if (fund < -0.003) longScore += 15;

    // OI fuel
    const oiScore = oiUsd > 0 ? Math.min(Math.log10(oiUsd + 1) * 3, 15) : 3;
    shortScore += oiScore;
    longScore += oiScore;

    // Price divergence boost
    if (fund > 0.0002 && c.change24h < -2) shortScore += 15;
    if (fund < -0.0002 && c.change24h > 2) longScore += 15;
  ...
  // Top 10 shorts (highest shortScore)
  const topShorts = scored
    .filter(c => c.shortScore >= 5)
  ...
  // Top 10 longs (highest longScore)
  const topLongs = scored
    .filter(c => c.longScore >= 5)
